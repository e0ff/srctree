...
pub fn router(ctx: *Frame) Route.RoutingError!Route.BuildFn {
    ...
    if (isHex(verb)) |_| {
        ...
        return if (ctx.request.method == .POST) updatePatch else view;
    }
...
pub fn simpleComment(line: []const u8, writer: *Writer, io: Io) !void {
    ...
    if (find(u8, line, "https://")) |link| {
        ...
        if (findScalarPos(u8, line, link, ' ')) |end| {
            ...
            try toBareLink(line[link..], writer);
        }
    } else {
